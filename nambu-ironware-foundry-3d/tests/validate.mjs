...
const required=['index.html','styles.css','first-scene-v4.css','manifest.webmanifest','sw.js','js/bootstrap.js','js/data.js','js/core.js','js/models.js','js/gesture.js','js/game.js','js/progressive.js','js/first-scene-v4.js','js/ui.js','js/main.js','icons/icon.svg','icons/maskable.svg','README.md','THIRD_PARTY_NOTICES.md'];
...
const bootstrap=fs.readFileSync(path.join(root,'js/bootstrap.js'),'utf8');for(const file of ['first-scene-v4.css','js/gesture.js','js/progressive.js','js/first-scene-v4.js'])assert.ok(bootstrap.includes(file),`bootstrap must load ${file}`);
const sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');for(const file of ['first-scene-v4.css','js/gesture.js','js/progressive.js','js/first-scene-v4.js'])assert.ok(sw.includes(file),`service worker must cache ${file}`);
const progressive=fs.readFileSync(path.join(root,'js/progressive.js'),'utf8');assert.ok(progressive.includes('single continuous packed-sand body'),'first chapter must use a continuous packed-sand surface');assert.ok(progressive.includes("surfaceType:'continuous-mesh'"),'debug snapshot must identify the continuous surface');assert.ok(!progressive.includes('t.rings=[]'),'stacked ring bodies must not return');
const v4=fs.readFileSync(path.join(root,'js/first-scene-v4.js'),'utf8');assert.ok(v4.includes("__NAMBU_VISUAL_REVISION__='4.0'"),'players must be migrated to visual revision 4');assert.ok(v4.includes('fresh physical scrape on wet mold sand'),'the first movement must create a physical fresh-cut trace');assert.ok(v4.includes('minimumGain'),'gesture movement must have an immediate minimum response');
const css=fs.readFileSync(path.join(root,'first-scene-v4.css'),'utf8');assert.match(css,/#loading:not\(\.active\)/,'loading screen must have an explicit inactive rule');assert.match(css,/pointer-events:none!important/,'inactive and decorative overlays must not intercept touch');assert.match(css,/#fallbackArt/,'transparent fallback art must leave hit testing');
...
console.log(`Validated ${N.STEPS.length} steps, ${N.SCENES.length} interactive scenes, ${N.CHAPTERS.length} chapters, continuous forming, and the first-scene input contract.`);
