(function(){
  'use strict';
  const REV='1.3.0',loading=document.querySelector('#loadingText'),fatal=document.querySelector('#fatal');
  function loadStyle(src){return new Promise((resolve,reject)=>{const l=document.createElement('link');l.rel='stylesheet';l.href=`${src}?v=${REV}`;l.onload=resolve;l.onerror=()=>reject(new Error(`style load failed: ${src}`));document.head.appendChild(l)})}
  ...
  async function boot(){
    try{
      await loadStyle('first-scene-v4.css');
      ...
      const files=['js/data.js','js/core.js','js/models.js','js/gesture.js','js/game.js','js/progressive.js','js/first-scene-v4.js','js/ui.js','js/main.js'];
