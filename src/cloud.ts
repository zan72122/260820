...
export class CloudBand {
  ...
  constructor(quality: number, seed = 777) {
    ...
    // ---- knot loops ----
    const loopUs = [0.521, 0.5, 0.479]; // loop 0 nearest the unicorn (leeward)
    for (let i = 0; i < LOOP_COUNT; i++) {
      ...
      mesh.rotateZ((i - 1) * 0.35 + (rand() - 0.5) * 0.3);
      mesh.rotateX((rand() - 0.5) * 0.22);
      ...
      this.loops.push({
        progress: 0, twist: 0, bounce: 0, jiggle: 0, working: 0,
      ...
      const oct = new THREE.OctahedronGeometry(0.09, 0);
  ...
  update(dt: number, time: number, cam: THREE.Camera, windAmp: number, sunUp: number, hornWorld: THREE.Vector3, hornNear: number) {
    ...
    for (let i = 0; i < this.sprites.length; i++) {
      ...
      const freeze = smoothstep(0.06, 0.16, Math.abs(s.u - 0.5));
      const speed = 0.006 * windAmp * lerp(freeze, 1, this.openAll);
      s.u += speed * dt * 6;
      if (s.u > 0.82) s.u = 0.18;
      const p = this.curve.getPointAt(clamp(s.u, 0, 1));
      const tan = this.curve.getTangentAt(clamp(s.u, 0, 1));
      const sideV = new THREE.Vector3().crossVectors(tan, new THREE.Vector3(0, 1, 0)).normalize();
      const wobble = Math.sin(time * 0.4 + s.phase) * 0.5;
      this.spriteDummy.position.copy(p)
        .addScaledVector(sideV, s.side * (1.5 + wobble * 0.6))
        .add(new THREE.Vector3(0, s.lift + Math.sin(time * 0.3 + s.phase * 2) * 0.3, 0));
