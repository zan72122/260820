 ...
 * The load path is meant to be readable: anchors → support lines → rungs → deck.
 */
...
export class Bridge {
  ...
  /** Deck centre-line point at s (0 near → 1 far), including sag + deflection. */
  ...
  /** Walkable surface query. Returns null when (x,z) is off the bridge corridor. */
  deckInfo(x: number, z: number): { s: number; y: number } | null {
    ...
    const t = clamp(((p.x - a.x) * ab.x + (p.z - a.z) * ab.z) / ab.lengthSq(), 0, 1);
    const c = this.centerAt(t);
  ...
  private rebuildLine(line: LineState): void {
    ...
    if (line.locked) {
      ...
      for (let i = 0; i < pts.length; i++) pts[i].y -= this.deflection(i / (pts.length - 1)) * 0.9;
    } else {
      // paying out: the free end droops as it flies across the gap
      const p = line.progress;
      const endS = p;
      const end = new THREE.Vector3().lerpVectors(a, this.far.clone().add(off), endS);
  ...
  weave(d: number): void {
    const before = this.weaveProgress;
  ...
  private buildRungs(): void {
    ...
    for (let i = 0; i < N_RUNGS; i++) {
      ...
      const pts = [left, mid, right];
      pts[0].add(new THREE.Vector3(0, 0, skew));
  ...
  closeDeck(d: number, colorSeq?: number[]): void {
    if (!this.deck) {
      ...
      this.deckGeo.setAttribute('position', new THREE.BufferAttribute(positions, 3));
  ...
  /** Register a live load at span fraction s (0..1), weight w (0..1). */
  ...
  addRail(sideIdx: 0 | 1, d: number, colorIdx: number): void {
    ...
    this.railColors[sideIdx].push(colorIdx);
  ...
  update(dt: number): void {
    // loads decay quickly — the weave springs back
    ...
    if (this.dirty) {
      this.rebuildDeck();
      for (const line of this.lines) if (line.locked) this.rebuildLine(line);
      this.dirty = false;
    }
  }
}
