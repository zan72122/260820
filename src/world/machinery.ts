...
function shaftPointAt(z: number, out = new THREE.Vector3()): THREE.Vector3 {
  const t = (z - SHAFT.zFrom) / (SHAFT.zTo - SHAFT.zFrom)
  return out.set(SHAFT.x, lerp(SHAFT.yFrom, SHAFT.yTo, t), z)
}
...
export class Machinery {
  ...
  private buildShaft(): void {
    ...
    const a = shaftPointAt(SHAFT.zFrom)
    const b = shaftPointAt(SHAFT.zTo)
    ...
    bar.castShadow = true
    this.shaftSpin.add(bar)

    // A rubber friction wheel opposite every drive roller.
    const wheelGeo = new THREE.CylinderGeometry(SHAFT.wheelRadius, SHAFT.wheelRadius, 0.024, 16)
    const hubGeo = new THREE.CylinderGeometry(0.026, 0.026, 0.034, 10)
    for (const r of ROLLERS) {
      if (!r.drive) continue
      const local = (r.z - mid.z) / Math.cos(pitch)
      const wheel = new THREE.Mesh(wheelGeo, m.rubber)
      wheel.position.y = local
      wheel.castShadow = true
      this.shaftSpin.add(wheel)
      const hub = new THREE.Mesh(hubGeo, m.darkSteel)
      hub.position.y = local
      this.shaftSpin.add(hub)
    }

    // Pillow-block bearings do not turn with the shaft.
    const merger = new MeshMerger()
    const blockGeo = new THREE.BoxGeometry(0.07, 0.09, 0.06)
    const strapGeo = new THREE.BoxGeometry(0.03, 0.16, 0.03)
    for (const z of [SHAFT.zFrom + 0.12, -0.02, 1.15, SHAFT.zTo - 0.14]) {
      shaftPointAt(z, _v)
      merger.addAt(blockGeo, buildMaterials().darkSteel, _v.x, _v.y, _v.z, 0, 0, 0, { cast: true })
      merger.addAt(strapGeo, buildMaterials().galvanised, _v.x, _v.y + 0.11, _v.z)
    }
  ...
  private buildGenerator(contacts: ContactShadows): void {
    ...
    // Heat-dissipation fins along the +X flank.
    for (let i = 0; i < 5; i++) {
      const fin = new THREE.Mesh(new THREE.BoxGeometry(0.03, s.y * 0.72, 0.02), m.housing)
      fin.position.set(s.x / 2 + 0.014, 0, -s.z / 2 + 0.07 + i * 0.07)
      g.add(fin)
    }

    // Four service screws on the inspection face.
    for (const sx of [-1, 1]) {
      for (const sy of [-1, 1]) {
        const screw = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.008, 0.012, 6), m.darkSteel)
        screw.rotation.z = Math.PI / 2
        screw.position.set(-s.x / 2 - 0.004, sy * (s.y / 2 - 0.035), sx * (s.z / 2 - 0.04))
        g.add(screw)
      }
    }

    // Shaft seal where the line shaft enters the -Z face.
    const seal = new THREE.Mesh(new THREE.CylinderGeometry(0.042, 0.05, 0.05, 12), m.darkSteel)
    seal.rotation.x = Math.PI / 2
    seal.position.set(0, -0.02, -s.z / 2 - 0.02)
    g.add(seal)

    // Inspection window with a mechanical needle behind glass, facing -X.
    const face = new THREE.Group()
    face.rotation.y = -Math.PI / 2
    face.position.set(-s.x / 2 - 0.002, 0.012, 0.03)
    g.add(face)

    const bezel = new THREE.Mesh(new THREE.CylinderGeometry(0.072, 0.072, 0.012, 20), m.darkSteel)
    bezel.rotation.x = Math.PI / 2
    face.add(bezel)

    const dial = new THREE.Mesh(new THREE.CircleGeometry(0.062, 24), m.dial)
    dial.position.z = 0.0075
    face.add(dial)
    ...
    for (let i = 0; i <= 8; i++) {
      ...
      const tick = new THREE.Mesh(
        new THREE.BoxGeometry(0.004, i % 4 === 0 ? 0.016 : 0.009, 0.002),
        m.darkSteel,
      )
      const r = 0.05
      tick.position.set(Math.sin(a) * r, Math.cos(a) * r, 0.0095)
      tick.rotation.z = -a
      face.add(tick)
    }

    this.needle.position.z = 0.011
    face.add(this.needle)
    const needleMesh = new THREE.Mesh(new THREE.BoxGeometry(0.006, 0.052, 0.003), m.needle)
    needleMesh.position.y = 0.02
    ...
    const glass = new THREE.Mesh(new THREE.CircleGeometry(0.064, 20), m.glassDark)
    ...
    const hoop = new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.018, 8, 20, Math.PI), m.galvanised)
  ...
  private buildDistributionBox(contacts: ContactShadows): void {
    ...
    CIRCUIT_IDS.forEach((id, i) => {
      ...
      if (id === 'path') {
        ...
        const mesh = new THREE.Mesh(geo, m.dial)
        ...
        const roofMesh = new THREE.Mesh(roofRelief(), m.dial)
        roofMesh.rotation.y = Math.PI
        relief.add(roofMesh)
        for (const s of [-1, 1]) {
          const post = new THREE.Mesh(new THREE.BoxGeometry(0.008, 0.03, 0.006), m.dial)
          post.position.set(s * 0.026, -0.014, -0.003)
          relief.add(post)
        }
      } else {
        const trunk = new THREE.Mesh(new THREE.BoxGeometry(0.01, 0.03, 0.007), m.dial)
        trunk.position.set(0, -0.022, -0.0035)
        relief.add(trunk)
        const crown = new THREE.Mesh(new THREE.SphereGeometry(0.027, 12, 8), m.dial)
  ...
  update(dt: number, shaftOmega: number, output: number, storage: number): void {
    ...
    this.shaftSpin.rotation.y = this.shaftAngle
    ...
    for (const lever of this.levers) {
      ...
      lever.pivot.rotation.x = lever.angle
      lever.pilot.emissiveIntensity = lever.thrown > 0 ? 1.4 : 0
    }
  }
}
