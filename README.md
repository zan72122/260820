...
背景の果樹園は、草・小石・落ち葉・遠景の幹・遠景の葉塊をすべて
`InstancedMesh` で描き、遠近 2 つのリングに分けて配置しています（近景は
カメラが向く楔形から外し、遠景はフォグへ沈める）。

一方で **texture atlas と baked lighting は使っていません**。理由は、
テクスチャが全て実行時の手続き生成で外部読み込みが 0 のため atlas 化の利得が
なく、光源が「太陽 + 空 + シートの反射」の 3 つだけで、そのうち反射は毎フレーム
変わるため焼き込めないからです。LOD も、遠景を別リング・別スケールで置く粗い
切り分けに留めています。より重い背景を足す場合は、ここが最初の最適化点です。
