...
await app.sim(0.5);
console.log('state', JSON.stringify(await app.state()).slice(0, 210));
await app.shot('probe-brushed-with');
await app.page.evaluate(() => {
  window.__imo.scene.traverse((o) => {
    if (o.material && o.material.customProgramCacheKey && o.material.customProgramCacheKey() === 'digsite-topsoil') o.visible = false;
  });
});
await app.shot('probe-brushed-without');
await app.close();
