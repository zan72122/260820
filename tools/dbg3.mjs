...
const page = await browser.newPage({ viewport: { width: 500, height: 500 }, deviceScaleFactor: 1 })
page.on('pageerror', e => console.log('PAGEERROR', e.message))
await page.goto('http://127.0.0.1:4173/?fast=1', { waitUntil: 'load' })
await page.waitForFunction(() => !!window.__chiffon)
const info = await page.evaluate(() => {
  window.__chiffon.goto('press')
  window.__chiffon.step(3)
  window.__dbg.isolateCake()
  return window.__dbg.cakeInfo()
})
console.log(JSON.stringify(info, null, 1))
await page.evaluate(() => window.__chiffon.render())
await page.screenshot({ path: SP + '/cake-iso.png' })
await browser.close()
